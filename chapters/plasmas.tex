...
\noindent with $m$ denoting the particle's rest mass.
...
\noindent We equate the first adiabatic invariant at an initial point, denoted by subscript 0, and a turning point, denoted by subscript 1. At the reflection point, the particle's kinetic energy will be entirely in the perpendicular mode:
\begin{eqnarray}
...
\end{equation}
where $R_e = 6378$ km is the radius of the Earth, and L is the \emph{L-shell} of interest (see section \ref{section:dipole_model}).
...
\begin{equation}
\mvec{n}\times \mvec{n} \times \vec{E} +  \mvec{\epsilon}\cdot\vec{E} = 0
\end{equation}
where $\mvec{n} = \vec{k}c/\omega$ is the wave refractive index. Assuming a wave propagating with some angle $\theta$ between $\mvec{n}$ and the background magnetic field, we arrive at:
\begin{equation}
\begin{pmatrix}
S - n^2\cos^2\theta & -iD & n^2\cos{\theta}\sin{\theta} \\
iD & S - n^2 & 0 \\
n^2\cos{\theta}\sin{\theta} & 0 & P - n^2\sin^2{\theta} \end{pmatrix}\begin{pmatrix}E_x \\ E_y \\ E_z\end{pmatrix} = 0
...
\begin{eqnarray}
&An^4 - Bn^2 + C &= 0 \label{eqn:disp_rln}  \\
&A& = S \sin^2\theta + P\cos^2\theta \\
&B& = RL\sin^2\theta + PS(1 + \cos^2\theta) \\
&C& = PRL
\end{eqnarray}

Equation \eqref{eqn:disp_rln} is biquadratic -- we can solve for $n^2 = k^2c^2/\omega^2$ using the quadratic formula.
...
\begin{equation}
n^2 = 1 - \bigfrac{\frac{\omega_{pe}^2}{\omega^2}}{1 - \frac{\omega_{ce}^2\sin^2\theta}{2(\omega^2 - \omega_{pe}^2)} \pm\big[\big(\frac{\omega_{ce}^2\sin^2\theta}{2(\omega^2 - \omega_{pe}^2)}\big)^2 + \frac{\omega_{ce}^2}{\omega^2}\cos^2\theta\big]^{1/2}}
...
Figure \ref{fig:whistler_mode_dispersion} shows a typical dispersion relation for a magnetospheric plasma (L $\approx$ 2) by plotting frequency vs wavenumber ($n = k c/\omega$). The whistler mode is the lower branch of the RHCP mode.
\begin{figure}[!t]
\begin{center}
